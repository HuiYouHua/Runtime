...
- (NSMethodSignature *)methodSignatureForSelector:(SEL)selector {
    ...
    if ([sel rangeOfString:@"set"].location == 0) {
        // v@:@ => [-(void)setName:(NSString *)name]
        // 第一字符v代表函数返回类型void, 第二字符@代表self的类型id,第三个字符:代表_cmd的类型SEL,第四字符@表示参数name
        return [NSMethodSignature signatureWithObjCTypes:"v@:@"];
    } else {
        // @@: => [-(NSString)name]
        // 第一字符@代表函数返回类型NSString，第二个字符@代表self的类型id，第三个字符:代表_cmd的类型SEL。
